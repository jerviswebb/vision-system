...
ALLOW_MODEL_INPUT_SIZE_MISMATCH="${ALLOW_MODEL_INPUT_SIZE_MISMATCH:-0}"
...
echo "  imgsz=${IMGSZ:-model-metadata-or-256-fallback} frame=${FRAME_WIDTH}x${FRAME_HEIGHT} interval_ms=$INTERVAL_MS"
...
if [ "$ALLOW_MODEL_INPUT_SIZE_MISMATCH" = "1" ] || [ "$ALLOW_MODEL_INPUT_SIZE_MISMATCH" = "true" ]; then
  echo "  WARNING: model input size mismatch override is enabled"
  ARGS+=(--allow-model-input-size-mismatch)
fi
